...
export default function AppShowcase() {
  ...
  // Em mobile, reduz o spread para caber na tela
        ...
        <div className="relative overflow-hidden">
          ...
          >
            ...
            {screens.map((s, i) => (
              <motion.div
                key={i}
                initial={{ rotate: s.rotate, x: s.x * scale, y: (s.y + 100) * scale, opacity: 0 }}
                whileInView={{ rotate: s.rotate, x: s.x * scale, y: s.y * scale, opacity: s.opacity }}
                whileHover={{ rotate: s.rotate, x: s.x * scale, y: (s.y - 14) * scale, opacity: Math.min(s.opacity + 0.18, 1) }}
